...
def collect_articles(full_refresh=False):
    seed_url = 'https://www.google.com/search?q=%22minor+league+cricket%22&tbm=nws&source=lmns&hl=en&start={start_idx}'
    if not full_refresh:
        seed_url += '&tbs=qdr:w'
    full_article_list = []
...
def crawl_articles(new_article_urls):
    ...
    for url in new_article_urls:
        try:
            ...
            continue
        b = article.text
        if len(b) > 0 and "minor league cricket" in b[:500].lower():
...
def summarize(article_text):
    ...
    system_prompt = '''
You are a helpful assistant highly skilled at summarizing a news article by extracting relevant information about the minor league cricket tournament from that article. 

You will be given a news article. Your job is to extract relevant information about the minor league cricket tournament from that article and summarizing it. 
...
def index_articles(articles):
    ...
    for article in articles:
        ...
        b = article['body']
        date = article['date']
        if len(date) > 0 and len(b) > 0 and "minor league cricket" in b[:500].lower():
            try:
...
if __name__ == '__main__':
    ...
    try:
        with open('milc_raw/article_list.txt') as f:
    ...
    if os.path.exists('milc_raw/lsh_unique.pickle'):
        print('Loading MinHashLSH index from disk')
        with open('milc_raw/lsh_unique.pickle', 'rb') as f:
    ...
    try:
        ...
        if len(new_article_urls) > 0:
          with open('milc_raw/article_list.txt', 'a') as f:
                f.write('\n'.join(new_article_urls) + '\n')
        print('Article list updated')
        ...
        if len(articles) > 0:
            ...
            output_folder = f'milc_raw/articles/{year}/{month}'
        ...
        with open('milc_raw/lsh_unique.pickle', 'wb') as f:
