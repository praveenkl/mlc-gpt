...
def summarize(article_text):
    print(f'Summarizing article: {article_text[:100]}...')
...
if __name__ == '__main__':
    # Read all the articles from the json files in the unique_articles folder and create a list of articles
    articles = []
    for root, dirs, files in os.walk('unique_articles'):
        for file in files:
            with open(os.path.join(root, file), 'r') as f:
                articles.extend(json.load(f))
    print(f'Total number of articles read from files: {len(articles)}')
    # create a small list of 10 articles
    articles = articles[:10]
    # print(articles[4]["summary"])
